...
contract DigitalAssetContract {
    ...
    struct customerAssetData {
        // AES-256
        bytes encryptedSymmetricKey; // 0x7465737400000000000000000000000000000000000000000000000000000000
        bytes32 encryptedFileHash; // fixed to 32 bytes
        string ipfsURI;
    }
    ...
    event hashConfirmed(bytes indexed encryptedSymmetricKey);
    ...
    function grantAccess(
        ...
        bytes32 encryptedFileHash, // fixed from 20 bytes to 32 bytes
    ...
    function compareHashesAndGetKey(bytes32 customerHash) external returns (bytes memory) {
        ...
        bytes memory encryptedSymmetricKeyToSend = customerAddrToData[msg.sender]
            .encryptedSymmetricKey;
        delete customerAddrToData[msg.sender];
        emit hashConfirmed(encryptedSymmetricKeyToSend);
